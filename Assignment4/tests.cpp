...
TEST(Transformer, check_constructor_overloading)
{
    Voice voice("English");
    Gun gun("AKM", 30, 90);
    Transformer transformer("Transformer", "Divine", 200, gun.get_gun_type(), gun.get_damage(), gun.get_ammo(), 100, voice);
...
TEST(Bazookron, test_class_bazookron)
{
    ...
    Bazookron bazookron("Bazookron", "Decepticon", 200, bazooka.get_gun_type(), bazooka.get_damage(), bazooka.get_ammo(), 50);

    EXPECT_EQ(bazookron.get_stabilization(), 100);
    EXPECT_EQ(bazookron.get_points_to_ulta(), 0);

    bazookron.set_stabilization(50);
